...
const CreateConference = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) throw new Error("Error al crear la conferencia");

      setShowSuccessModal(true);
    } catch (error) {
      alert("Hubo un problema al crear la conferencia.");
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 p-4">
      {showSuccessModal && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg text-center w-96">
            <h2 className="text-2xl font-bold text-green-600">¡Conferencia Creada!</h2>
            <p className="text-gray-700 mt-2">Tu conferencia se ha registrado con éxito.</p>
            <button
      ...
      <div className="fixed inset-0 flex items-center justify-center">
        <div className="w-full max-w-2xl mx-4 bg-white rounded-lg shadow-lg p-6 overflow-y-auto max-h-[90vh]">
          <h1 className="text-3xl font-bold text-center text-gray-900 mb-6">Crear Conferencia</h1>
